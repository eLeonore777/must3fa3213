...
def banner():
            ...
            print (f"""\n\n\n\n                  -   {R}#    {W}Coder: Sayco {R}  #   {W} -\n   {R}              -{W} Telegram: https://t.me/saycou {R}-\n                  {R}#{W}   Fb-sayco  Version: 1.0 {R} #\n\n""")
...
class run:
	...
	def runall(sayco):
		if Check_Email(sayco.email).all() == True:
			if facebook(sayco.email).rest() == True:
				return True
			else:
				return False
		else:
...
class start:
	def __init__(sayco):
	           ...
	           time.sleep(2)
	def st(sayco):
		email = rand_sayco().get_email_sayco()
		W = "\033[0m"
		G = '\033[32;1m'
		R = '\033[31;1m'
		print (f"	{G} ~ {W} Total Email: {len(email)}")
		print (f"	{G} ~ {W} Telegram : https://t.me/saycou \n")
		time.sleep(2)
		for line in email:
				...
				if status == True:
					...
					if sayco.mode == 'telegram':
						url = f'https://api.telegram.org/bot{sayco.token}/sendMessage?chat_id={sayco.id}&text=• Available (Facebook) :\n\n•` {line} `\n\n• Telegram: @saycou&parse_mode=markdown'
						saycou.get(url)
